...
char* map_rrd_file_update(char *input_file);
...
int rrd_update(
    ...
    char **argv)
{
    printf("RRD_UPDATE.C: entered RRD_update");
    struct option long_options[] = {
        {"template", required_argument, 0, 't'},
 ...
 ////////////////////////////////////////////////////////////////////
    printf("RRD_UPDATE.C:ARGV[OPTIND] IS %s\n", argv[optind]);
    char* local_file_full = argv[optind];
    char* ts1 = strdup(local_file_full);
    char* ts2 = strdup(local_file_full);
    char* local_file = basename(ts2);
    char* dir = dirname(ts1);

    char* changed_file = map_rrd_file_update(local_file);
    strcat(dir,"/");
    strcat(dir,changed_file);
    printf("RRD_UPDATE.C: Writing to: %s\n", dir);
////////////////////////////////////////////////////////////////////
...
char* map_rrd_file_update(char *input_file)
{
	...
	char value[100];
	int n;
	char *new_filename=(char*)malloc(100);
	char *arr[10]; 
	...
	else
	{
		while(fgets(line, sizeof(line), fr) != NULL)
		   {
			n = split_update(line, arr);
			strcpy(key,arr[0]);
			strcpy(value,arr[1]);
			if(!strcmp(input_file,key))
			{
				strcpy(new_filename,value);
...
int split_update(char *str, char *arr[10]){
    int beginIndex = 0;
    int endIndex;
    int maxWords = 10;
    int wordCnt = 0;
